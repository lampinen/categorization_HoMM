...
lang_homm_subdirs = c("results_123/language_HoMM")
num_runs = 5
```
...
ggplot(data=comparison_d %>%
  ...
  annotate("text", x=1.5, y=1.1, alpha=0.5, label="p < 0.001 (both perm- and t-test)") +
...
ggplot(data=comparison_d,
  ...
  annotate("text", x=2.5, y=1.1, alpha=0.5, label="n.s. (both ps > 0.05)") +
  scale_y_continuous(breaks = c(0.5, 0.75, 1), labels = c("50%", "75%", "100%")) +
  guides(color=F)
