...
ggplot(data=comparison_d %>% 
         filter(!grepl("examples", run_type)) %>%
         mutate(run_type = ifelse(run_type == "Language", "Language", "HoMM")),
       aes(x=run_type, 
           color=run_type,
           y=mean_accuracy)) +
  geom_hline(yintercept=1, linetype=2, alpha=0.5) +
  ...
  annotate("text", x=0.62, y=1.03, alpha=0.5, label="Optimal") +
  annotate("path",x=c(1,1,2,2),y=c(1.05,1.07,1.07,1.05),alpha=0.5) +
  annotate("text", x=1.5, y=1.1, alpha=0.5, label="p < 0.05") +
  scale_y_continuous(breaks = c(0.5, 0.75, 1), labels = c("50%", "75%", "100%")) +
  guides(color=F)

ggsave("../../metamapping_paper/figures/concepts_adaptation.png", width=4, height=3)
```
...
ggplot(data=lang_arch_d,
  ...
  geom_point(stat="summary",
             fun.y="mean",
             size=4) +
  scale_color_manual(values=c("#073e88", "#477ec8")) +
