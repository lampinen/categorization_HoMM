...
ggplot(data=comparison_d %>%
  ...
  geom_density() +
  scale_color_manual(values=c("#e41a1c", "#477ec8")) +
  labs(x="Generalization accuracy", y="Density") +
  # scale_x_continuous(breaks = c(2, 4, 8, 12, 16),
  #                    labels = c("4\n(~110)", "8\n(~150)", "16\n(~200)", "24\n(~250)", "32\n(~300)")) +
  # scale_y_continuous(breaks = c(0.5, 0.75, 1), labels = c("50%", "75%", "100%")) + #, limits = c(0.8, NA)) +
  guides(color=guide_legend(title=NULL)) +
...
ggplot(data=comparison_d %>%
  ...
  geom_point(stat="summary",
             ...
             position=position_dodge(0.8)) +
  scale_color_manual(values=c("#e41a1c", "#477ec8")) +
  labs(x="Number of training meta-mappings\n(Approximate number of training concepts)", y="Percent of runs with > 99% generalization accuracy") +
  scale_x_continuous(breaks = c(2, 4, 8, 12, 16),
                     labels = c("4\n(~110)", "8\n(~150)", "16\n(~200)", "24\n(~250)", "32\n(~300)")) +
  scale_y_continuous(breaks = c(0., 0.5, 1), labels = c("0%", "50%", "100%")) + #, limits = c(0.8, NA)) +
  guides(color=guide_legend(title=NULL)) +
  theme(legend.position=c(0.8, 0.33))#, legend.box.background=element_rect(), legend.box.margin=margin(1, 6, 6, 6))
```