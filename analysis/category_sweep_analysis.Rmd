...
```{r}
parent_dir = "../categorization_HoMM_size_sweep"
subdirs = c(
            "results_ntrain_150",
            "results_ntrain_200",
            "results_ntrain_250",
...
ggplot(data=comparison_d %>%
  ...
  labs(x="Number of training concepts", y="Accuracy on evaluation tasks") +
...
ggplot(data=comparison_d %>%
  ...
  labs(x="Number of training concepts", y="Adapted concept accuracy") +
  # annotate("text", x=0.6, y=0.52, alpha=0.5, label="Chance") +
  # annotate("text", x=0.85, y=1.02, alpha=0.5, label="Optimal adaptation") +
  # annotate("path",x=c(1,1,1.95,1.95),y=c(1.05,1.08,1.08,1.05),alpha=0.5) +
  # annotate("text", x=1.5, y=1.1, alpha=0.5, label="n.s. (both perm- and t-test)") +
  # annotate("path",x=c(2.05, 2.05, 3, 3),y=c(1.05,1.08,1.08,1.05),alpha=0.5) +
  # annotate("text", x=2.5, y=1.1, alpha=0.5, label="p < 0.05 (both tests)") +
  scale_y_continuous(breaks = c(0.8, 0.9, 1), labels = c("80%", "90%", "100%"), limits = c(0.8, NA)) +
  guides(color=guide_legend(title=NULL)) +
  theme(legend.position=c(0.8, 0.2))#, legend.box.background=element_rect(), legend.box.margin=margin(1, 6, 6, 6))
...
ggplot(data=comparison_d %>%
         filter(mapping_toe == "metamapping_is_eval",
                run_type != "HoMM from examples",
                num_train_examples > 200) %>%  # for fewer, we often don't have enough support for eval tasks, they're missing from most runs
         mutate(run_type = ifelse(run_type == "HoMM from language", "HoMM", "Language\nGeneralization")),  
       aes(x=num_train_examples, 
           color=run_type, 
           y=mean_accuracy)) +
  ...
  # annotate("path",x=c(2.05, 2.05, 3, 3),y=c(1.05,1.08,1.08,1.05),alpha=0.5) +
  # annotate("text", x=2.5, y=1.1, alpha=0.5, label="p < 0.05 (both tests)") +
  scale_y_continuous(breaks = c(0.9, 0.95, 1), labels = c("90%", "95%", "100%"), limits = c(0.9, NA)) +
  scale_x_continuous(breaks = c(250, 300)) +
  guides(color=guide_legend(title=NULL)) +
  theme(legend.position=c(0.8, 0.2))#, legend.box.background=element_rect(), legend.box.margin=margin(1, 6, 6, 6))
